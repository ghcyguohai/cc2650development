                             ...
                             nrf_uart_event_handler_t      event_handler)
{
    ...
    if (m_cb.handler)   // if handler is not null, the config the interrupt 
    {
...
void UART0_IRQHandler(void)
{
    ...
    (
        ...
        if (nrf_uarte_event_check(NRF_UARTE0, NRF_UARTE_EVENT_RXTO)) // if rx timeout 
        {
            nrf_uarte_event_clear(NRF_UARTE0, NRF_UARTE_EVENT_RXTO);
            if (m_cb.rx_buffer_length)  // check the whether the legth is valid  or not 
            {
    ...
    CODE_FOR_UART(
        ...
        if (nrf_uart_event_check(NRF_UART0, NRF_UART_EVENT_TXDRDY))   // this is used to transmmit
        {
