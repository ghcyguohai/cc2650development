...
uint32_t app_get_rxfifo_data_nums(void)
{
  if (FIFO_LENGTH(m_rx_fifo) == m_rx_fifo.buf_size_mask)
    {
        uint32_t err_code = nrf_drv_uart_rx(rx_buffer,1);
...
void app_read_rxfifo_datas(uint8_t* pbuf,uint32_t len)
{
    uint32_t read_size=len;
    app_fifo_read(&m_rx_fifo,pbuf,(uint32_t*)read_size);
}
