...
{  
    ...
    for( ; usDataLen > 0; usDataLen--){  
        crc8 = CRC8Table[crc8^*puchMsg];  
        puchMsg++;  
    }  
    return(crc8);  
...
{
 uint8_t caculate_crc=0;

	if(len>2)
	{
	  caculate_crc=Get_Crc8(src,len-1);
	  if(caculate_crc==src[len-1])
	  {
        return 0;
	  }
	  else
...
uint8_t Check_Crc16(uint8_t* src,uint16_t len)
{
 ...
 {
   caculate_crc=Get_Crc16(src,len);
   if(caculate_crc==src[len-2])
   {
     return 0;
   }
   else
...
void Attach_Crc8(uint8_t* src,uint8_t len)
{
 ...
 caculate_crc=Get_Crc8(src,len);
 src[len]=caculate_crc;
}
